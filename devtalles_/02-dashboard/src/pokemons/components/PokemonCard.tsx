...
export const PokemonCard = ({ pokemon }: Props) => {
  ...
  const isFav = useAppSelector((state) => !!state.pokemons.fav[id]); // transforma un objeto que va a estar en el staore en la parte del state en fav en un boolean
        ...
        <div className="bg-gray-300 rounded overflow-hidden shadow-lg ">
          <div className="flex flex-col items-center p-6 bg-transparent">
            <Image
              ...
              priority={
                false
              } /* esto hace que sea un lazyloading. Solo si está en falses */
            />

            <p className="pt-2 text-lg font-semibold text-gray-950 capitalize">
              {name}
            </p>
            <div className="mt-5">
              <p className="mb-2 text-gray-800 font-medium tracking-wide">
                More info
              </p>
              <div className="flex flex-row gap-3">
                <Link
                  href={`/dashboard/pokemonId/${id}`}
                  className="border border-gray-700 rounded-full py-2 px-4 text-xs font-semibold text-gray-800"
                >
                  by Id
                </Link>
                <Link
                  href={`/dashboard/pokemonName/${name}`}
                  className="border border-gray-700 rounded-full py-2 px-4 text-xs font-semibold text-gray-800"
                >
                  by Name
                </Link>
